...
export async function GET(request) {
  try {
    ...
    const { data: cached, error: cacheReadError } = await admin.from("achievement_games").select("hero_artwork_url, logo_artwork_url").eq("user_id", user.id).eq("provider", provider).eq("provider_game_id", gameId).maybeSingle();
    ...
    // Older deployments may not have the optional artwork-cache columns yet.
    // Artwork lookup should still succeed; it simply remains uncached in Supabase.
    if (!cacheReadError) {
