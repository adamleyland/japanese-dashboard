...
export function normalizeShadowingSettings(settings = {}) {
  return {
    ...
    playbackRate: Math.max(0.5, Math.min(1.5, toNumber(settings.playbackRate, DEFAULT_SHADOWING_SETTINGS.playbackRate))),
...
export function buildShadowingQueue(cards, settings = {}, offset = 0) {
  ...
  const normalizedOffset = ((Math.floor(Number(offset) || 0) % orderedCards.length) + orderedCards.length) % orderedCards.length;
  const rotatedCards = normalizedOffset
    ? [...orderedCards.slice(normalizedOffset), ...orderedCards.slice(0, normalizedOffset)]
    : orderedCards;

  return rotatedCards.slice(0, normalizedSettings.sentenceCount);
}
...
export function calculateShadowingProgress(currentIndex, currentRepetition, totalCards, totalRepetitions) {
  ...
  const completedSteps = boundedIndex * totalRepetitions + boundedRepetition;
