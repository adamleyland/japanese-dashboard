...
function weightedShuffleItems(items, getWeight = () => 1) {
  return [...items]
    .map((item, index) => {
      const weight = Math.max(0.01, Number(getWeight(item)) || 1);
...
function normalizeIdCounts(values) {
  if (!Array.isArray(values)) {
    return new Map();
  }
...
function buildBalancedQueue({
  ...
  const preferredVideoIdSet = normalizeIdSet(preferredVideoIds);
  const skipped = {
    duplicate: 0,
    currentQueue: 0,
    recent: 0,
  ...
  if (preferredVideoIdSet.size) {
    for (const [channelId, videos] of videosByChannel.entries()) {
      videosByChannel.set(
        channelId,
        weightedShuffleItems(videos, (video) => (preferredVideoIdSet.has(video.id) ? 1.45 : 1)),
      );
    }
  }
  ...
  console.info("[YouTube Queue] Generated balanced queue", {
    ...
    skipped,
    preferredVideoCount: preferredVideoIdSet.size,
...
async function verifyYoutubeAccountAccess(accessToken, reason = "account-bundle") {
  console.info("[YouTube OAuth] Channel verification start", {
    reason,
  });
  const requestUrl =
  ...
  const accountProfile = extractAccountProfile(data);
  console.info("[YouTube OAuth] Channel verification success", {
    reason,
    hasAccountProfile: Boolean(accountProfile),
    channelId: accountProfile?.channelId || "",
  });
...
async function fetchAccountQueueVideos(
  ...
  const selectedChannelIdSet = normalizeIdSet(selectedChannelIds);
  const preferredChannelCounts = normalizeIdCounts(preferredChannelIds);
  ...
  const channelIds = weightedShuffleItems(
    eligibleChannels.map((channel) => channel.channelId || channel.id).filter(Boolean),
    (channelId) => 1 + Math.min(preferredChannelCounts.get(channelId) || 0, 5) * 0.18,
  ...
  console.info("[YouTube Queue] Preparing account queue candidates", {
    ...
    preferredChannelCount: preferredChannelCounts.size,
    preferredChannelVoteCount: preferredChannelIds.length,
    preferredVideoCount: normalizeIdSet(preferredVideoIds).size,
  });
...
export async function fetchYouTubeAccountBundle(
  ...
  const accountVideos = await fetchAccountQueueVideos(
    ...
    subscriptionResult?.channelDetailsMap || new Map(),
    {
      ...
      preferredChannelIds: options.preferredChannelIds || [],
      preferredVideoIds: options.preferredVideoIds || [],
    },
  );
