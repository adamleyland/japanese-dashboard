...
export const DEFAULT_GAMING_LAYOUT_MODE = "artwork";
...
export function formatLastPlayedDate(value) {
  const timestamp = toTimestamp(value);
  if (!timestamp) {
    return "No date";
  }
...
export function formatBritishOrdinalDate(value) {
  const timestamp = toTimestamp(value);
  ...
  const monthAndYear = date.toLocaleDateString("en-GB", { month: "short", year: "numeric" });
...
export function getDeviceLabel(game = {}) {
  ...
  if (game?.source === "local" || game?.source === "steam-deck") {
    const labels = [...new Set(deviceCandidates.map((device) => {
      ...
      if (normalizedDevice.includes("steam deck") || normalizedDevice.includes("steamdeck")) {
        return "Steam Deck";
      }

      return toSafeString(device);
    }).filter(Boolean))];
