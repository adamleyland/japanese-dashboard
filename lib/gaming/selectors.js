...
export function computeOverallGamingTotal(games) {
  return games.reduce(
    (totals, game) => {
      ...
      if (game.includeInOverallTotal !== false) {
...
export function selectCurrentlyPlayingGame(games) {
  ...
  return [...games].sort(compareByFreshness)[0] || null;
}

export function selectVisibleGames(
  games,
  { sortKey = "hours-played", sourceFilter = "all" } = {},
) {
  const filteredGames =
    sourceFilter === "all" ? games : games.filter((game) => game.source === sourceFilter);
