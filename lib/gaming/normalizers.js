...
function pickXboxPlatform(game = {}) {
  const devices = pickXboxDevices(game);
  const deviceMatch = devices.find((device) => {
    const normalizedDevice = toSafeString(device).toLowerCase();
    return normalizedDevice.includes("pc") || normalizedDevice.includes("windows");
  });
  ...
  const xboxDeviceMatch = devices.find((device) => {
    const normalizedDevice = toSafeString(device).toLowerCase();
...
export function normalizeXboxGame(game = {}) {
  ...
  return {
    ...
    platform: getPlatformLabel(pickXboxPlatform(game)),
