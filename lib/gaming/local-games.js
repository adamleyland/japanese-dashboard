...
export function normalizeLocalGame(game = {}) {
  ...
  const isSteamDeck = metadata?.tracker_source === "steam-deck";
  ...
  return {
    source: isSteamDeck ? "steam-deck" : "local",
    ...
    includeInOverallTotal: isSteamDeck,
    launchUrl: isSteamDeck ? null : buildSteamShortcutLaunchUrl(steamShortcutId),
    platform: isSteamDeck ? "Steam Deck" : platforms[0] || "Local",
    devices: platforms,
    metadataProvider: toSafeString(game?.metadata_provider, "steam-shortcut"),
    raw: game,
  };
}
